...
fs.readdirSync(taskLibDir).forEach(function (taskConfigFile) {
    if (/\.(js|jsx|ts|tsx)$/.test(taskConfigFile)) {
...
taskFiles.forEach(function (taskFile) {
    babel.transformFile(taskFile, {
        ...
        if (/\.(js|jsx|ts|tsx)$/.test(taskFile)) {

            var taskFileFilter = taskFile.match(/\/(.*.(js|jsx|ts|tsx))$/);
            if (!taskFileFilter) {
                taskFileFilter = taskFile.match(/\\(.*.(js|jsx|ts|tsx))$/);
            }
        ...
        const fileName = `${path.parse(taskFile || '').name}.${suffix}`;

        fs.writeFileSync(LIB_PATH + '/' + fileName, result.code, {
            encoding: 'utf-8'
        });

        console.log(`${taskLibDir}/${taskFile} ---> ${LIB_PATH}/${fileName} ${'done'.green}`);
    });
});
...
function copyScss(fileUrl) {
    ...
    waitCopyFiles.forEach(function (file) {
        ...
        fs.writeFileSync(LIB_PATH + '/' + file, scssFile, {
            encoding: 'utf-8'
        });

        console.log(`${taskLibDir}/${file} ---> ${LIB_PATH}/${file} ${'done'.green}`);
    });
